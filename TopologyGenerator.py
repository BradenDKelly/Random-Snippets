"""
Written by Braden Kelly sometime in 2018 (this is being written in 2020)
The purpose is to take a pdb (ultimately, the purpose is to take anything)
...
TODO:
    1) Make the geom opt a stand along part
    2) Take as input other file types
        a) use babel/obabel to convert internally
    3) Produce force-fields other than GAFF
    4) Optionally do a conformer search
        a) add in rdkit functionality
        b) find conformer search that is (free) and can do implicit solvent
...
parser.add_argument('-dat', type=str, default='ff99SB',
                    help='files used by GAFF for generating parameters, default is ff99SB')
...
parser.add_argument('-qob', type=str, default="AUG-cc-pVTZ",
                    help='geometry optimization QM basis set. Default is aug-cc-pVTZ')
parser.add_argument('-res', type=str, default="MOL", help='default residue name of molecule, default is MOL')
parser.add_argument('-GOpt', type=bool, default=False,
                    help='whether or not do run a geometry optimization, default is False')
parser.add_argument('-mo', type=int, default=3000, help='memory to use for Gaussian Optimization, default is 3000 mb')
parser.add_argument('-npp', type=int, default=6, help='number of processors to use for Gaussian, default is 12')
parser.add_argument('-dial', type=str, default="Water",
                    help='dielectric for SCRF calculation, default is water. Enter numerical value')
...
def _gaussianInputFile(Name, QMtheory, QMbasis, pop, charge, mult):
    f = open(Name + ".dat", 'w')
    f.write('%mem={}MB\n'.format(mo))
    f.write('%nproc={} \n'.format(npp))
    ...
    if chargeMethod.lower() == "resp":
        if implicitBackground:
            f.write(
                '#T {0}{1} pop={2} TEST density=current SCRF=({3}) SCF=tight iop(6/33=2) iop(6/42=6) iop(6/50=1)\n'.format(
                    QMtheory, QMbasis, pop, dialKeyword))
        else:
            f.write('#T {0}{1} pop={2} TEST density=current SCF=tight iop(6/33=2) iop(6/42=6) iop(6/50=1)\n'.format(
                QMtheory, QMbasis, pop))
    ...
    coords, atype = _getPDBMolCoords(Name, length)

    for i in range(length):
        f.write('{0} {1:7.7} {2:7.7} {3:7.7} \n'.format(atype[i], coords[i][0], coords[i][1], coords[i][2]))
    f.write(' \n')
    if chargeMethod == "resp":
        if implicitBackground and inp.dial.lower() != "water":
            f.write('eps={} \n\n'.format(inp.dial))
        f.write(molName + ".gesp")
        f.write(' \n \n'.format())
        f.write(molName + ".gesp")
        f.write(' \n \n'.format())
    elif implicitBackground and inp.dial.lower() != "water":
        # f.write( 'Alpha=0.5 \n' )
        f.write('eps={} \n\n'.format(inp.dial))
        # f.write( 'epsinf=1.77 \n\n' )
    f.close()
...
def _gaussianOpt(Name, QMOptTheory, QMOptBasis, charge, mult):
    f = open(Name + "Opt" + ".dat", 'w')
    f.write('%mem={}MB\n'.format(mo))
    f.write('%nproc={} \n'.format(npp))
    f.write('%chk={0:s}Opt\n'.format(Name))
    f.write(' \n')
    if implicitBackground:
        f.write('#T {0}/{1} Opt Freq Density=Current SCRF=(Solvent=Water)\n'.format(QMOptTheory,
                                                                                    QMOptBasis))  # use implicit solvent
    else:
        f.write('#T {0}{1} Opt Freq Density=Current \n'.format(QMOptTheory, QMOptBasis))
    # Density=Current Opt # other options
    f.write(' \n')
    f.write('Remark Section')
    f.write(' \n \n'.format())
...
def _getPDBMolCoords(Name, length):
    pdb = open(Name + ".pdb")
    coords = np.zeros((length, 3))
    atype = []
    iter = 0
...
def GenerateZMatrixFile(filename):
    ...
    if background == True:  # This is taken from QMMM_Interface, and so there shouldn't be solCoords...
        for i, item in enumerate(solCoords):
            f.write('{0:7.7} {1:7.7} {2:7.7} {3:7.7} \n'.format(solCoords[i][0], solCoords[i][1], solCoords[i][2],
                                                                chargeGroup[i]))
    f.write(' \n')
    f.write('{0}.wfx \n\n'.format(molName))
    f.close()
...
molName = inp.sm  # solute molecule
pop = inp.p  # population analysis method i.e., ESP or MBS (mulliken)
QMtheory = inp.t  # "HF"  #HF/6-31G*  # B3LYP # HF
if inp.t == "AM1" or inp.t == "PM6":
    QMbasis = ""
else:
    QMbasis = "/" + inp.b  # "6-31G(d)" #"AUG-cc-pVTZ", "6-31G*"
QMprogram = inp.g  # g09/g16
mult = inp.mult  # multiplicity of molecule
ffType = inp.ff  # gaff
# goutput      = inp.o
inpname = inp.gauss
end = inp.log
chargeMethod = inp.cm  # Needed to distinguish Iterative Hirshfeld, Iterative CM5, ddec6, MBIS etc...

charge = inp.chge
QMOptTheory = inp.qot
QMOptBasis = "/" + inp.qob
residueName = inp.res
GaussOpt = inp.GOpt  # do a geometry optimization first?
mo = inp.mo  # amount of memory to use in Gaussian
npp = inp.npp  # number of processors to use in Gaussian
ffDat = inp.dat
implicitBackground = inp.i  # use implicit SCRF background solvent in QM

if inp.dial.lower() == "water":
...
if GaussOpt:
    ...
    cmd = 'python ~/scripts/python/modify_pdb.py -sm {}'.format(
        molName)  # this modifies the pdb to a nicer format... atoms get numbered rather than a column of C C H C O C N, you have C1 C2 H1 C3 O1 C4 N1
    p = sub.Popen(cmd, shell=True, stderr=sub.STDOUT, stdout=sub.PIPE).communicate()[0]
    print(p.decode())
####################################################################
coordFileType = []
...
if chargeMethod.lower() == "resp":
    ...
    cmd = 'module load gcc/5.4.0; module load amber/18; antechamber -i {0:s}.gesp -fi gesp -o {1:s}.mol2 -fo mol2 -c {2:s} -at {3:s} -nc {4} -pf y'.format(
        molName, molName, chargeMethod, ffType, charge)
    p = sub.Popen(cmd, shell=True, stderr=sub.STDOUT, stdout=sub.PIPE).communicate()[0]
    print(p.decode())
elif ("bcc") in chargeMethod.lower():
    print("Running Antechamber to get AM1BCC charges...")
    cmd = 'module load gcc/5.4.0; module load amber/18;antechamber -i {0:s}.pdb -fi pdb -o {1:s}.mol2 -fo mol2 -c {2:s} -at {3:s} -nc {4} -pf y '.format(
        molName, molName, chargeMethod, ffType, charge)
    p = sub.Popen(cmd, shell=True, stderr=sub.STDOUT, stdout=sub.PIPE).communicate()[0]
    print(p.decode())
    ...
    cmd = 'module load gcc/5.4.0; module load amber/18;antechamber -i {0:s}.pdb -fi pdb -o {1:s}.mol2 -fo mol2 -c bcc -at {3:s} -nc {4} -pf y'.format(
        molName, molName, chargeMethod, ffType, charge)
    p = sub.Popen(cmd, shell=True, stderr=sub.STDOUT, stdout=sub.PIPE).communicate()[0]
    print(p.decode())
    ...
    GenerateHortonFile(
        "mbis")  # generates the... Horton ... file... it is kind of obvious..., takes as input the name of the python script to be produced
    # Now run Horton
    ...
    # Step 1)
    print("Step 1) call antechamber for dummy mol2 file")
    cmd = 'module load gcc/5.4.0; module load amber/18;antechamber -i {0:s}.pdb -fi pdb -o {1:s}.mol2 -fo mol2 -c bcc -at {3:s} -nc {4:d} -pf y'.format(
        molName, molName, chargeMethod, ffType, charge)
    p = sub.Popen(cmd, shell=True, stderr=sub.STDOUT, stdout=sub.PIPE).communicate()[0]
    print(p.decode())
...
### Now use parmchk """
cmd = 'module load gcc/5.4.0; module load amber/18;parmchk2 -i {0:s}.mol2 -f mol2 -o {1:s}.frcmod'.format(molName,
                                                                                                          molName)
p = sub.Popen(cmd, shell=True, stderr=sub.STDOUT, stdout=sub.PIPE).communicate()[0]
print(p.decode())
...
#
########################################
cmd = 'module load nixpkgs/16.09 intel/2016.4 openmpi/2.1.1 python/2.7.14 scipy-stack/2019a gaussian; python ~/acpype.tgz/acpype/acpype.py -p {0:s}.prmtop -x {1:s}.inpcrd'.format(
    molName, molName)
p = sub.Popen(cmd, shell=True, stderr=sub.STDOUT, stdout=sub.PIPE).communicate()[0]
print(p.decode())
...
if "bcc" in QMtheory.lower() or "pm" in QMtheory.lower():  # if semiempirical, don't write out basis... there isn't one...
    ...
    cmd = 'cp MOL_GMX.top {}_{}_{}_{}_{}_{}_{}_GMX.top'.format(molName, chargeMethod.lower()
                                                               , QMtheory, str(inp.b), ffType, implicitBackground,
                                                               inp.dial)
    p = sub.Popen(cmd, shell=True, stderr=sub.STDOUT, stdout=sub.PIPE).communicate()[0]
